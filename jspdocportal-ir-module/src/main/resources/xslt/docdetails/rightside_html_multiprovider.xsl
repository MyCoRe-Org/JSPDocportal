...
<xsl:stylesheet version="3.0" 
  ...
  <xsl:template match="/">
   ...
   <!-- Dauerhaft zitieren -->
    ...
    <!-- multiple provider are possible on bundle -->
    ...
    <xsl:variable name="catalogs">
      <xsl:for-each select="$class_provider">
        <xsl:value-of select="mcrmods:to-mycoreclass(., 'single')/categories/category/label[@xml:lang='x-catalog']/@text" />
      </xsl:for-each>
      <xsl:if test="not($class_provider)">
        ...
        <xsl:if test="/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:recordInfo/mods:recordInfoNote[@type='k10plus_ppn']">
          ...
          <p>
            ...
            <xsl:for-each select="/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:identifier[@type='vd18']">
              <a class="badge px-1" target="_blank" href="https://kxp.k10plus.de/DB=1.65/CMD?ACT=SRCHA&amp;IKT=8080&amp;TRM=VD18{replace(.,' ','+')}">VD18</a>
            </xsl:for-each>
